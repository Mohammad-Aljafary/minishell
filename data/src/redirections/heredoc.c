...
/*   Updated: 2025/05/30 14:43:32 by mkurkar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// System call error - continue shell execution
...
// System call error occurred - heredoc creation failed
// Free the filename and return NULL to indicate failure
// Error message was already printed in open_heredoc()
char	*check_file(t_token *node, t_all *all)
{
	...
	result = open_heredoc(&join, node, all);
	if (result != 0)
	{
		free(join);
		return (NULL);
